...
def main():
    ...
    if args.light:
        LIGHT_MODE = True
        verbose_print("Light mode, for small list.")
    if args.full:
        FULL_MODE = True
        verbose_print("Full mode. Warning, the output could be very heavy.")
    if args.output:
    ...
    if args.users is not None:
        ...
        if file_or_not == True:
            process_file_passwd(args.password, output_file_name)
            sys.exit(0)
        else:
            process_input_passwd(args.password, output_file_name)
            sys.exit(0)
